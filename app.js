...
// Base v0.3.1 + Feature 1 (report graph header) + Feature 2 (cabled edges) + Feature 3 (node click detail) + Feature 4 (DT tree animated)
...
(function() {
  ...
  navTabs.forEach(function(tab) {
    tab.addEventListener('click', function() {
      ...
      else if (target === 'portfolio') renderPortfolioImpact();
    });
  });
  ...
  // ========================================================================
  // GLOBAL SPHERE (Feature 2: cabled edges + Feature 3: node click)
  // ========================================================================
  function renderGlobalGraph(svgId) {
    ...
    // FEATURE 2: cabled edges (colored, thick by weight)
    ...
    // Nodes (sized by centrality if available)
  ...
  // FEATURE 3: node click -> rotation animation + detail panel
  ...
  // FEATURE 1: compact subgraph for report header
  ...
  function renderL1ReportCenter() {
    ...
    // FEATURE 1: persistent graph in report header
    var graphHeaderHtml =
  ...
  function renderFooter() {
    ...
    if (STATE.flow === 'l1-report') {
      footer.classList.remove('tall');
      var s = STATE.activeScenario;
      footerRight.innerHTML = '<span class="ft-item"><span class="ft-lbl">LEVEL</span> L1 &middot; RAG</span><span class="ft-sep">&middot;</span><span class="ft-item amber"><span class="ft-lbl">COST</span> ' + s.l1Report.cost + '</span><span class="ft-sep">&middot;</span><span class="ft-watermark">Draft -- not calibrated forecasts</span><span class="ft-sep">&middot;</span><span class="ft-status ok">ANSWER READY</span>';
      return;
    }
  ...
  var DOSSIER_MAIN_KEYS = [
    'ukraine-main', 'taiwan-main', 'iran-main',
    'eu-china-trade-main', 'us-debt-hyperscalers-main',
  ...
  function horizonMultiplier(scenarioKey, horizon) {
    ...
    if (!mat) return 0;
    var m = mat.maturationByHorizon[horizon];
    if (!m) return 0;
    if (horizon === '0-12') {
      if (m === 'materialising' || m === 'active' || m === 'escalating' || m === 'ongoing' || m === 'building' || m === 'campaigns') return 1.0;
      return 0.4;
    } else if (horizon === '12-24') {
      if (m === 'consolidated' || m === 'critical' || m === 'structural' || m === 'tension peak' || m === 'recalibration' || m === 'outcomes') return 1.15;
      return 0.7;
    } else {
      if (m === 'stable' || m === 'new equilibrium' || m === 'consolidated' || m === 'structural' || m === 'post-cycle configuration' || m === 'resolution or break') return 0.85;
      return 0.5;
    }
  }
